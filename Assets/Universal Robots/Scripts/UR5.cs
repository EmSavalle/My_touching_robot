...
public class UR5 : MonoBehaviour
{
	...
	IEnumerator Initialize()
	{
		...
		//target.localPosition = targetObject.localPosition;

		target.localRotation = tcp.localRotation;
		//target.localRotation = targetObject.localRotation;
	...
	public float IsRobotMoving()
	{
		return (float)ur.GetTCPSpeed()[0] + (float)ur.GetTCPSpeed()[1] + (float)ur.GetTCPSpeed()[2];	
	}
    ...
    public void goPosition()
    {
		...
		double[] q = { Mathf.Deg2Rad * 5, Mathf.Deg2Rad * -65, Mathf.Deg2Rad * 130, Mathf.Deg2Rad * -155, Mathf.Deg2Rad * 90, Mathf.Deg2Rad * 0 };
		MoveToConfiguration(q);
    }
